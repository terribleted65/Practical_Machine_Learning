...
collect a large amount of personal activity data relatively inexpensively. 
...
the specified execution of the exercise, while the other four classes correspond to 
common mistakes. 
...
the columns that have data.  Now the testing and training dataframes only have 
54 columns each.
...
I split the remaining training dataframe into a training set (70%) and a testing 
or valdation dataset (30%). 
...
(0.09%).  The model with 10 folds and 50 trees provided an accuracy of 99.42% with 
a significant p-value of 2.2e-16 and a very small OOB error rate of 0.86%.

I then applied the cross validated model against the provided test dataset. The
results below will also be written out to separate files for evaluation.
